use super::{default_chance, default_reduction, AkcConfig, FriendInfo};
...
pub fn increase_chances_by_unit(
    config: &mut AkcConfig,
    unit_added_chance: f64,
...
mod test {
    ...
    fn test_get_unknown_names() {
        ...
        let names = vec!["John".to_owned(), "Doe".to_owned(), "Jane".to_owned()];
    ...
    fn test_increase_chances_by_unit() {
        ...
        assert_eq!(
            config.friends[0].chance,
            default_chance::AJI + 0.1 * default_chance::AJI
        );
        assert_eq!(config.friends[1].chance, default_chance::KI);
        assert_eq!(
            config.friends[2].chance,
            default_chance::CHI + 0.1 * default_chance::CHI
        );
    }
